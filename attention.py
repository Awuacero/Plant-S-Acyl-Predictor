...
from tf_keras.layers import Layer, InputSpec  # Actualizado desde keras.engine
import numpy as np
...
class myFlatten(Layer):
    ...
    def compute_output_shape(self, input_shape):
        # Corregimos el output_shape para reflejar la dimensión exacta
        return (input_shape[0], self.mydeletedim)
    
    def call(self, x, mask=None):
        # Recortamos el tensor
        x = x[:, :self.mydeletedim]
        # Forzamos explícitamente la forma para que Dense sepa su tamaño
        x = tf.reshape(x, [-1, self.mydeletedim])
        return x
